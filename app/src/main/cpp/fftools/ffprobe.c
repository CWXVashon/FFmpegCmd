...
static struct section sections[] = {
        ...
        [SECTION_ID_FRAME_SIDE_DATA] =     {SECTION_ID_FRAME_SIDE_DATA, "side_data", 0,
                                            {SECTION_ID_FRAME_SIDE_DATA_TIMECODE_LIST, -1}},
        [SECTION_ID_FRAME_SIDE_DATA_TIMECODE_LIST] =     {SECTION_ID_FRAME_SIDE_DATA_TIMECODE_LIST,
                                                          "timecodes", SECTION_FLAG_IS_ARRAY,
                                                          {SECTION_ID_FRAME_SIDE_DATA_TIMECODE,
                                                           -1}},
        [SECTION_ID_FRAME_SIDE_DATA_TIMECODE] =     {SECTION_ID_FRAME_SIDE_DATA_TIMECODE,
                                                     "timecode", 0, {-1}},
...
void printf_json(char *fmt, ...) {
    ...
    AVBPrint avbPrint[4];
    av_bprint_init(avbPrint + 0, 0, 1);
    av_bprint_init(avbPrint + 1, 0, 1);
    av_bprint_init(avbPrint + 2, 0, 1);
    av_bprint_init(avbPrint + 3, 0, 65536);

    av_vbprintf(avbPrint + 3, fmt, args);

    int length = snprintf(print_buffer + buffer_length, buffer_size - buffer_length, "%s%s%s%s",
                          avbPrint[0].str, avbPrint[1].str, avbPrint[2].str, avbPrint[3].str);

    av_bprint_finalize(avbPrint + 0, NULL);
    av_bprint_finalize(avbPrint + 1, NULL);
    av_bprint_finalize(avbPrint + 2, NULL);
    av_bprint_finalize(avbPrint + 3, NULL);
...
static void json_print_section_header(WriterContext *wctx) {
    ...
    if (section->flags & SECTION_FLAG_IS_WRAPPER) {
        ...
        if (section->flags & SECTION_FLAG_IS_ARRAY) {
            ...
            printf_json("\"%s\": {%s", buf.str, json->item_start_end);
        } else {
            printf_json("{%s", json->item_start_end);

            /* this is required so the parser can distinguish between packets and frames */
            if (parent_section && parent_section->id == SECTION_ID_PACKETS_AND_FRAMES) {
                if (!json->compact)
                    JSON_INDENT();
                printf_json("\"type\": \"%s\"", section->name);
            }
        }
        av_bprint_finalize(&buf, NULL);
    }
...
static inline void json_print_item_str(WriterContext *wctx,
    ...
    av_bprint_init(&buf, 1, AV_BPRINT_SIZE_UNLIMITED);
    printf_json("\"%s\":", json_escape_str(&buf, key, wctx));
    av_bprint_clear(&buf);
    printf_json(" \"%s\"", json_escape_str(&buf, value, wctx));
    av_bprint_finalize(&buf, NULL);
}
...
static void json_print_int(WriterContext *wctx, const char *key, long long int value) {
    ...
    av_bprint_init(&buf, 1, AV_BPRINT_SIZE_UNLIMITED);
    printf_json("\"%s\": %lld", json_escape_str(&buf, key, wctx), value);
    av_bprint_finalize(&buf, NULL);
}
...
static void print_dynamic_hdr10_plus(WriterContext *w, const AVDynamicHDRPlus *metadata) {
    ...
    if (metadata->targeted_system_display_actual_peak_luminance_flag) {
        ...
        for (int i = 0; i < metadata->num_rows_targeted_system_display_actual_peak_luminance; i++) {
            for (int j = 0;
                 j < metadata->num_cols_targeted_system_display_actual_peak_luminance; j++) {
...
static const OptionDef real_options[] = {
        ...
        {"print_filename", HAS_ARG, {.func_arg = opt_print_filename},
         "override the printed input filename", "print_file"},
